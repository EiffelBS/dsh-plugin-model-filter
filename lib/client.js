...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ModelSelect({ locked, available, directory, load, select, t }) {
			...
			if (query === "") {
				...
				for (const group of state.groups) {
					if (group.name.toLowerCase().includes(query) || group.id.toLowerCase().includes(query)) {
						filteredGroups.push(group);
						continue;
					}
					const models = group.models.filter((model) => model.name.toLowerCase().includes(query) || model.id.toLowerCase().includes(query));
						...
						children: [
							...
							pane === "model" && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
								...
								state.status === "ready" && filteredGroups.length === 0 && (0, react_jsx_runtime.jsx)("div", {
									...
									children: query === "" ? t("empty.models") : t("empty.noMatch")
								})
							] }),
		...
		* (activated last, so it wins the single-slot election) with the
		* searchable ModelSelect; search-only keys fall back to the built-in
